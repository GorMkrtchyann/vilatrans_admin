...
const FeaturesEdit = (props) => {
    ...
    useEffect(() => {
        axios.get('http://localhost:8080/home/get-features').then(res => {
            if (res.data) {
    ...
    const fileUpload = async (fileObj) => {
        ...
        if (file) {
            if (file.type.startsWith('image/')) {
                ...
                if (getSizes.width !== 1920 || getSizes.height !== 851) {
                    setError('Image dimensions must be 1920x851.');
    ...
    const Submit = async (data) => {
        ...
        const res = await axios.put('http://localhost:8080/home/update-features', sendingData);
        ...
        <div className={'features-side'}>
            <form className={'features-form'} onSubmit={handleSubmit(Submit)}>
                <div className={'features-box-form'}>
                    <div>
                        <p>Image dimensions must be 1920x851.</p>
                        <FileUploadInput
                            onChange={fileUpload}
                            text={'Upload Image'}
                ...
                <div className={'features-form-btn'}>
                    <VilaButton text={`Save`} loadingText={'Saving'} loading={loading}/>
                </div>
            </form>
            {
                reqData ? <FeaturesEditReview reqData={reqData} htmlContents={htmlContents}/> : <LinearProgress/>
            }
        </div>
    );
}
