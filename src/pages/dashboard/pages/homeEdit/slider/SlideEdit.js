...
const SlideEditForm = ({editSlide, setAllData, setEditSlide}) => {
    ...
    const Submit = async (data) => {
        ...
        if (editSlideObj){
            const res = await axios.put(`http://localhost:8080/home/slide-edit`, {
        ...
        const res = await axios.post('http://localhost:8080/home/slide', sendingData);
        if (res){
            reset();
            setVideo(null);
            setImage(null);
...
export const SlideEdit = () => {
    ...
    useEffect(() => {
        axios.get('http://localhost:8080/home/slide-data').then(data => {
