...
const OfficesInfoForm = ({setInfosArr, editObj, setAddForm, setEditObj}) => {
    ...
    const Submit = (data) => {
        ...
        if (editObj) {
            axios.put('http://localhost:8080/contact/editOfficeInfo',
                {
                    id: editObj._id,
                    ...sendingObj
                }
        ...
        axios.post('http://localhost:8080/contact/addOfficeInfo',
            sendingObj
        ).then(res => {
            if (!res.data.error) {
                setInfosArr(res.data)
            ...
            <div className={'officesInfo__form__inputs'}>
                <div>
                    ...
                    <TextField
                        ...
                        variant="standard"
                        defaultValue={editObj ? editObj.name.am : null}
                        {...register('nameArm')}
                        required
                    />
                    <TextField
...
const DeleteDialog = ({setOpen, id, setInfosArr}) => {
    ...
    const Delete = () => {
        axios.put('http://localhost:8080/contact/deleteOfficeInfo',
            {
                id: id
            }
        ).then(res => {
    ...
    return (
        <div className={'officesInfo__details'}>
            ...
            <ul>
                ...
                <li><span><IconBuildingSkyscraper/> Name With Armenian:</span> {obj.name.am}</li>
                ...
                <li><span><IconMapPin/> Address With Armenian:</span> {obj.address.am}</li>
...
export const OfficesInfo = () => {
    ...
    useEffect(() => {
        axios.get('http://localhost:8080/contact/getAllOfficeInfo'
        ).then(r => {
            if (!r.data.error) {
                setInfosArr(r.data)
            }
